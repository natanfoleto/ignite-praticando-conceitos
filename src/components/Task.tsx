    ...
    <div className={styles.task}>
      <div className={styles.checkbox}>
        <button onClick={() => toggleTask(task.title)}>
          {task.checked ? (
            <img src={Check} alt="Desmarcar" />
          ) : (
            <img src={Uncheck} alt="Marcar" />
          )}
        </button>

        <p className={task.checked ? styles.checked : styles.unchecked}>
          {task.title}
        </p>
      </div>

      <button className={styles.delete} onClick={() => deleteTask(task.title)}>
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
